...
def get_author(file_path) -> str:
    ...
    student_id: str = file_name.split('_')[1][:10]
    text_file_name: str = file_name.split('_')[0] + '_' + file_name.split('_')[1] + '_' + file_name.split('_')[2] + '_' + file_name.split('_')[3] + '.txt'
    text_file_path: str = os.path.join(os.path.dirname(file_path), text_file_name)
...
def get_file_dict(folder_path) -> dict[str, str]:
    file_dict: dict[str, str] = {}
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            if file.endswith('.docx') or file.endswith('.pdf'):
                file_path: str = os.path.join(root, file)
                author: str = get_author(file_path)
                text: str = get_file_text(file_path)
